...
import api.transaction_api.models as models
from api.transaction_api.models  import Transaction
import api.transaction_api.schemas as schemas
from api.database import get_db
from api.user_api.service import get_current_user
...
async def transactions(db:Session=Depends(get_db),current_user = Depends(get_current_user)):
    ...
    try:
        ...
        if not result:
            raise HTTPException(status_code=status.HTTP_204_NO_CONTENT,detail="no transaction")
        return result
...
async def get_transaction(id:int,current_user = Depends(get_current_user),db:Session=Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(status_code=204,detail=f"not found")
...
async def get_day_transactions(
    ...
    try:
        ...
        if not transactions:
            raise HTTPException(status_code=200,detail="no transaction")
        return {
            "date": date_,
             "total_amount": total_amount,
            "transactions": transactions
